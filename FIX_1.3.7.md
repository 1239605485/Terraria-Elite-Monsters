...
在部分 Android IL2CPP 构建中，`Player.Update` 和 `Player.UpdateBiomes`
Hook 都会在进入世界时触发 `SIGABRT`。因此 1.3.9 移除了所有 Player 方法
Hook。
...
- 版本升级为 `1.3.9`（`2026090205`），确保 TEFManager 不会继续使用旧版本。
