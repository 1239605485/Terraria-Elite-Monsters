...
6. 将下载得到的 ZIP 导入 TEFManager，确认安装的是 0.6.0 红色名称修复版。

本版本的旅途、经典、专家、大师、传奇模式概率均为 100%，并在 `Main.MouseText` 绘制入口强制让精英怪名称使用红色“【精英】”标识。测试完成后，将 `EliteMonsters/mod.c` 中的概率数组改回正式值并重新运行 Actions：
