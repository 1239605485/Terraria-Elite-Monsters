# EliteMonsters 1.0.2：100% 概率与AI强化测试版
...
注意：这只代表每个符合条件的普通 NPC 都会尝试转化；友好 NPC、城镇 NPC、Boss 和无效 NPC 仍会被过滤。由于所有普通敌怪都可能变成精英怪，测试时可能出现敌怪强度明显升高的情况。1.0.2 会在启动时枚举实际 AI 方法；如果成功，应在日志中看到 `NPC AI enhancement hook installed`。
