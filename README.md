...
- 传奇环境匣仍按原设计保持 30% 独立掉落概率；单次未掉落不代表异常。
...
- 传奇精英在原版掉落处理完成后，有 **30%** 概率额外掉落 1 个原版环境匣；困难模式后使用对应的困难模式环境匣。
...
- 当前源码默认将四档游戏模式的精英生成概率设置为 100%，用于测试。
...
1.2.0 会优先直接获取当前游戏的 `Terraria.NPC.AI()`，并让 TEFKernel
